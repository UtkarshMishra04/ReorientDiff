...
def get_camera_image(
    ...
    segm = np.array(segm).reshape(height, width)
    rgb = rgba[:, :, :3]
...
def draw_lines_from_poses(pose, color=None):
    ...
    # calculate end_point of line segment of length 0.1 from start_point and angles (in 3D)

    end_point = start_point + 0.1 * np.array(
        [
            np.cos(euler_angles[0]),
            np.cos(euler_angles[1]),
            np.cos(euler_angles[2]),
        ]
    )
