...
from models.language.core.clip import build_model, load_clip, tokenize

from models.language.core import fusion
from models.language.core.fusion import FusionConvLat
...
class CLIPLingUNetLat(nn.Module):
    ...
    def encode_text_batch(self, x):
        with torch.no_grad():
            tokens = []
            for i in range(len(x)):
                tokens.append(tokenize([x[i]]))
            tokens = torch.cat(tokens, dim=0).to(self.device)
            text_feat, text_emb = self.clip_rn50.encode_text_with_embeddings(tokens)
