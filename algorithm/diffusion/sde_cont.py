...
from algorithm.diffusion.utils.cont_utils import sample_v, log_normal, sample_vp_truncated_q
import numpy as np
...
class VariancePreservingSDE(torch.nn.Module):
    ...
    def alpha(self, t):
        # print("calculating for t=", t.squeeze().ndim)

        if t.squeeze().ndim == 0:
            if t.squeeze() == 1:
                t = t*257/256

        # print(self.cosine_schedule(t).squeeze()[0])
        # assert False
        return self.cosine_schedule(t) #/self.cosine_schedule(t=torch.zeros_like(t))
...
class PluginReverseSDE(torch.nn.Module):
    ...
    def __init__(self, scorefunc, beta_min=0.1, beta_max=20.0, T=1.0, t_epsilon=0.001, N = 256, vtype='rademacher', debias=False):
        super().__init__()
    ...
    def dsm(self, x, condition):
        ...
        # print(pred_x0.shape, target.shape, std.shape, x.shape, y.shape)
    ...
    def elbo_random_t_slice(self, x):
        ...
        score = self.x0_2_epsilon(t_, y)
        mu = self.base_sde.g(t_, y) * score - self.base_sde.f(t_, y)
        ...
        Nu = - (score ** 2).reshape(x.size(0), -1).sum(1, keepdim=False) / 2 / qt
        yT = self.base_sde.sample(torch.ones_like(t_) * self.base_sde.T, x)
        lp = log_normal(yT, torch.zeros_like(yT), torch.zeros_like(yT)).reshape(x.size(0), -1).sum(1)
