...
def get_grasp_poses_eval(env, segm):
    # segm = env.obs["segm"]
    depth = env.obs["depth"]
    K = env.obs["K"]
    segm = segm.reshape(*depth.shape)
    mask = (segm==1) & (~np.isnan(depth))
    pcd_in_camera = reorientbot.geometry.pointcloud_from_depth(
        depth, fx=K[0, 0], fy=K[1, 1], cx=K[0, 2], cy=K[1, 2]
    )
    normals_in_camera = reorientbot.geometry.normals_from_pointcloud(
...
def plan_place(env, target_grasp_poses):
    ...
    for grasp_pose in target_grasp_poses:
        ...
        succesful_grasp_poses.append((grasp_pose, result))
    ...
    if len(succesful_grasp_poses) == 0:
        return {}, []

    result = succesful_grasp_poses[0][1]

    return result , succesful_grasp_poses


def plan_place_eval(env, target_grasp_poses):
    obj_to_world = pp.get_pose(env.fg_object_id)
