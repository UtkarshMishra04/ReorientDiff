...
class ReorientationDataset(torch.utils.data.Dataset):
    def __init__(self, data_path=DEFAULT_DATA_PATH, device='cuda'):
        ...
        # print('Pose mean', self.pose_mean.shape)
        # print('Pose std', self.pose_std.shape)
        # print('Grasp mean', self.grasp_mean.shape)
        # print('Grasp std', self.grasp_std.shape)
        # print('Reorient mean', self.reorient_mean.shape)
        # print('Reorient std', self.reorient_std.shape)
    ...
    def parse_file(self, file_path):
        ...
        try:
            ...
            # select reorientation prediction for multiple of 8

            num_reorients = reorientation_prediction.shape[0]
            num_reorients = num_reorients - (num_reorients % 8)
            
            # choose num_reorients random reorientation predictions randomly

            rand_selection = np.random.choice(np.arange(reorientation_prediction.shape[0]), num_reorients, replace=False)
            reorientation_prediction = reorientation_prediction[rand_selection]

            for prompt in text_prompt:
                for reorient_pose in reorientation_prediction:
                    file_data.append((rgb_image, self.prompt2str(prompt), rgbd_image, pose, object_onehot, target_grasp_poses, reorient_pose, segmentation, pile_file))
    ...
    def __getitem__(self, idx):
        ...
        reorientation_prediction = torch.Tensor((reorientation_prediction - self.reorient_mean) / (self.reorient_std + self.min_reorient_std + 0.001)) #.to(self.device)
        ...
        return image, text, pose, rgbd, object_onehot, target_grasps[index_gra, :], reorientation_prediction, segmentation, pile_file
    ...
    def parse_prediction(self, pose=None, target_grasps=None, reorientation_pred=None, object_onehot=None):
        ...
        if reorientation_pred is not None:
            reorientation_pred = reorientation_pred * (self.reorient_std[:] + self.min_reorient_std + 0.001) + self.reorient_mean[:]
